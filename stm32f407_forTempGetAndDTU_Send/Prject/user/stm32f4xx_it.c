...
void EXTI1_IRQHandler(void)
{
  ...
  if(EXTI_GetFlagStatus(EXTI_Line1))
	{
		if(GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_1)==Bit_RESET && GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_2)==Bit_SET)   // 低电平 
		{
			EXTI->EMR &=  (~EXTI_Line2);   // 关闭第二路中断
		}else if(GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_1)==Bit_SET && GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_2)==Bit_RESET){    //  高电平
				ItUp++;
				EXTI->EMR |=  EXTI_Line2;
		}
  }
...
void EXTI2_IRQHandler(void)
{
	...
	{
		if(GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_2)==Bit_RESET && GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_1)==Bit_SET)   // 低电平 
		{
			EXTI->EMR &=  (~EXTI_Line1);   // 关闭第二路中断
		}else if(GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_2)==Bit_SET && GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_1)==Bit_RESET){    //  高电平
		  ItUp--;
			EXTI->EMR |=  EXTI_Line1;
		}
	}
