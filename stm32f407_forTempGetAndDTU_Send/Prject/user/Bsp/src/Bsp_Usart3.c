...
//	GPIO_ResetBits(GPIOC,GPIO_Pin_1);
   ...
   USART_DeInit(USART3);    
   // PB10,PB11 
   RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD, ENABLE);    // 开启 IO 口时钟
   RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3, ENABLE);      // 开始串口 3 时钟 
   GPIO_PinAFConfig(GPIOD, GPIO_PinSource8, GPIO_AF_USART3);   //开启 串口3 复用时钟和配置 复用引脚 
   GPIO_PinAFConfig(GPIOD, GPIO_PinSource9, GPIO_AF_USART3);   //开启 串口3 复用时钟和配置 复用引脚 
